...
검증 명령별 의미와 자주 나는 로컬 문제 해결 방법은 [사용법 가이드](docs/USAGE.md)의 로컬 검증 섹션을 참고하세요.
...
이 저장소는 테스트용으로 Level 3 자가 개선 루프를 실험합니다. Codex가 만든 PR은 `CI / test`와 `Redteam Review / redteam-review` check가 성공하고, workflow 내부의 `Auto Merge Guard` 정책 평가가 허용해야 자동 merge 후보가 됩니다.
